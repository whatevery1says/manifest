...
class collection(jsl.Document):
    ...
    # Instantiate all Corpus properties
    ...
    publications = jsl.ArrayField(pattern='^,Publications,[a-zA-Z-_,]+,$',required=True)
    collectors = jsl.ArrayField(as_ref=True, required=True)
...
schema = collection.get_schema(ordered=True)
